<template>
  <div>
    <Card size="small" title="动态表单示例" class="!mb-4">
    ...
    <Card size="small" class="mt-5" title="componentProps动态改变">
      <BasicForm @register="register1" @submit="submitHandler" />
    </Card>
  </div>
</template>
<script lang="ts">
...
export default defineComponent({
  components: { BasicForm, [Button.name]: Button, Card },
  setup() {
    const [
      register,
      { setProps, updateSchema, appendSchemaByField, removeSchemaByField },
    ] = useForm({
      labelWidth: 120,
      schemas,
      actionColOptions: {
    ...
    const submitHandler = (values: Recordable) => {
      console.log(values, "////");
    };
    return {
      register,
